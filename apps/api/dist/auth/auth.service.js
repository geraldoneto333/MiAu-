"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AuthService = void 0;
const common_1 = require("@nestjs/common");
const jwt_1 = require("@nestjs/jwt");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const config_1 = require("@nestjs/config");
const usuario_entity_1 = require("../entities/usuario.entity");
let AuthService = class AuthService {
    constructor(usuariosRepo, jwtService, config) {
        this.usuariosRepo = usuariosRepo;
        this.jwtService = jwtService;
        this.config = config;
    }
    verifyPassword(plain, stored) {
        return plain === stored;
    }
    hashPassword(password) {
        return password;
    }
    createToken(username) {
        const minutes = Number(this.config.get('JWT_EXPIRE_MINUTES', '120'));
        const access_token = this.jwtService.sign({ sub: username }, { expiresIn: minutes * 60 });
        return { access_token, token_type: 'bearer' };
    }
    async register(dto) {
        const existing = await this.usuariosRepo.findOne({
            where: [{ username: dto.username }, { email: dto.email }],
        });
        if (existing) {
            throw new common_1.BadRequestException('Usuário ou Email já cadastrado no sistema');
        }
        const user = this.usuariosRepo.create({
            username: dto.username,
            email: dto.email,
            senhaHash: this.hashPassword(dto.password),
        });
        await this.usuariosRepo.save(user);
        return this.createToken(dto.username);
    }
    async login(username, password) {
        const user = await this.usuariosRepo.findOne({ where: { username } });
        if (!user || !this.verifyPassword(password, user.senhaHash)) {
            throw new common_1.UnauthorizedException('Usuário ou senha incorretos');
        }
        return this.createToken(user.username);
    }
    getProfile(user) {
        return { username: user.username, email: user.email };
    }
    async updateProfile(user, dto) {
        try {
            await this.usuariosRepo.update(user.id, {
                username: dto.username ?? user.username,
                email: dto.email ?? user.email,
                ...(dto.password
                    ? { senhaHash: this.hashPassword(dto.password) }
                    : {}),
            });
            return { message: 'Perfil atualizado com sucesso' };
        }
        catch (e) {
            throw new common_1.BadRequestException(`Erro ao atualizar: ${e}`);
        }
    }
};
exports.AuthService = AuthService;
exports.AuthService = AuthService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(usuario_entity_1.Usuario)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        jwt_1.JwtService,
        config_1.ConfigService])
], AuthService);
//# sourceMappingURL=auth.service.js.map